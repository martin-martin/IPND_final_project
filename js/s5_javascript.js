function sayHi() {
    // this function was the result of some of my steps towards achieving the checkbox
    // functionality. I liked it, and therefore kept it in.
    var y = document.getElementById("lonely_check").checked;
    // the problem here was that expanding it onto more checkboxes with a for loop
    // didn't work, because only the state of the first one would count.
    if (document.getElementById("lonely_check").checked) {
        alert("(^_^) - i also like it.");
    } else {
...
function incrementProgressBar(num) {
    ...
    var perc_list = [5, 3, 10, 20, 20, 20, 20, 2];
...
function normalize() {
    ...
    y.innerHTML = "Steps to finish the IPND:";
}